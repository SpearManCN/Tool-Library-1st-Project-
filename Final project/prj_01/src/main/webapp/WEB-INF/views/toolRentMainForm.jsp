...
<script>
    ...
    function init(){
		...
		<c:if test="${!empty param.selSortBtn}">
		if(${param.selSortBtn}==0){$("[name=sortBtn]").val("정렬-분류순");}
		if(${param.selSortBtn}==1){$("[name=sortBtn]").val("정렬-오름차순");}
		if(${param.selSortBtn}==2){$("[name=sortBtn]").val("정렬-내림차순");}
    ...
    function goSort(){
    	var sortNo = $("[name=selSortBtn]").val();
    	if(sortNo==0){
    		$("[name=selSortBtn]").val(1);
    	
    		search();
    		
    	}
    	if(sortNo==1){
    		$("[name=selSortBtn]").val(2);
    	
    		search();
    		
    	}
    	if(sortNo==2){
    	//	$("[name=sortBtn]").val("정렬-분류순");
    		$("[name=selSortBtn]").val(0);
...
<input type="hidden" name="selSortBtn" value="${param.selSortBtn}">
</form>
        <form name="sendForm"  action="/toolRentMainForm.do" method="post">
            ...
            <table  class="section" border="0" cellpadding="5" cellspacing="0" style="border-collapse:collapse" align="center" width="600px">
                <tr>
                    ...
                    <th><input name="sortBtn" type="button" onclick="goSort();" value="정렬-분류순">
                    </th>
                    <th>
